...
namespace bgslibrary
{
  ...
  bool VideoAnalysis::setup(int argc, const char **argv)
  {
    ...
    const char* keys =
		"{hp|help|false|Print help message}"/*简称，文件来源，文件值和帮助语句*/
      "{uf|use_file|false|Use video file}"
      "{fn|filename||Specify video file}"		//指定视频路径 dataset/video.avi
      ...
      ;
    cv::CommandLineParser cmd(argc, argv, keys);//命令行解析类，方便调用keys的内容
    ...
    if (use_file)
    {
	  ...
	  cout << "获取到视频zq调试" << filename << endl;//此处获取到dataset/video.avi
      if (filename.empty())
      {
  ...
  void VideoAnalysis::start()
  {
    ...
    do
    {
      ...
      frameProcessor->init();//创建各个BGS方法对象
